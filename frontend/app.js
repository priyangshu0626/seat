...
// ─── CORE SEATING ENGINE (BACKEND-POWERED) ───────────────────────
// Optimization is handled by the Python FastAPI backend.
// This frontend calls POST /generate-bulk to get arrangements.
// All constraint solving, scoring, beam search happen server-side.
...
// Schedule state — populated by backend API calls
let scheduleList = [];
...
async function buildScheduleFromBackend(upToDayIndex) {
  ...
  try {
    ...
    // Append results to local cache
    for (const day of data.days) {
      scheduleList.push(day.arrangement);
      // Update local seat counts
      for (let s = 0; s < PEOPLE.length; s++) {
        scheduleSeatCounts[day.arrangement[s]][s]++;
      }
      // Update local pair counts
      getPairs(day.arrangement).forEach(p => {
        schedulePairCounts[p] = (schedulePairCounts[p] || 0) + 1;
      });
    }
  } catch (err) {
...
function getDynamicRow(dayIndex, dateStr) {
  ...
  // Synchronous access — schedule must be pre-built via buildScheduleFromBackend
  if (scheduleList.length > dayIndex) {
...
async function recalculateHistory() {
  ...
  // Count how many class days we need
  let maxClassDay = 0;
  const base = new Date(START_DATE + 'T00:00:00');
  for (let i = 0; i <= totalDays; i++) {
    const d = new Date(base);
    d.setDate(d.getDate() + i);
    if (!isBlocked(fmtDate(d))) maxClassDay++;
  }

  // Pre-fetch the full schedule from backend (+ future buffer)
  const fetchTarget = maxClassDay + maxFutureDays + 10;
  await buildScheduleFromBackend(fetchTarget);
...
async function init() {
  ...
  // Fetch schedule from Python backend (async)
