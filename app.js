...
const ALL_PERMUTATIONS = permute([0, 1, 2, 3, 4]);

function scoreArrangement(arr) {
  let score = 0;
  const pairs = getPairs(arr);
  pairs.forEach(p => {
    score += (pairHistory[p] || 0) * 10;
  });
  // Edge fairness mapping
  score += (edgeCountsHistory[arr[0]] || 0) * 5;
  score += (edgeCountsHistory[arr[4]] || 0) * 5;
  return score;
}
...
function updateHistory(arr) {
  const pairs = getPairs(arr);
  pairs.forEach(p => {
    pairHistory[p] = (pairHistory[p] || 0) + 1;
  });
  edgeCountsHistory[arr[0]] = (edgeCountsHistory[arr[0]] || 0) + 1;
  edgeCountsHistory[arr[4]] = (edgeCountsHistory[arr[4]] || 0) + 1;
}

function getBestArrangement() {
  let best = null;
  let bestScore = Infinity;
  ALL_PERMUTATIONS.forEach(arr => {
    const score = scoreArrangement(arr);
    if (score < bestScore) {
      bestScore = score;
      best = arr;
    }
...
function getDynamicRow(dayIndex) {
  // Build sequentially to guarantee deterministic caching regardless of which future day is requested first
  for (let i = 0; i <= dayIndex; i++) {
    if (!generatedScheduleCache[i]) {
      const best = getBestArrangement();
      updateHistory(best);
      generatedScheduleCache[i] = best;
    }
  }
  return generatedScheduleCache[dayIndex];
}
...
function render(animate) {
  ...
  const row = getDynamicRow(dayIndex);
  ...
  cl.innerHTML = '';
  const roundStartIdx = dayIndex - (dayIndex % PEOPLE.length);
  for (let d = 0; d < PEOPLE.length; d++) {
    const r = getDynamicRow(roundStartIdx + d);
...
function computeEdgeCounts() {
  ...
  history.forEach(h => {
    const row = getDynamicRow(h.dayIndex);
    counts[row[0]]++;
    counts[row[4]]++;
  });
  return counts;
